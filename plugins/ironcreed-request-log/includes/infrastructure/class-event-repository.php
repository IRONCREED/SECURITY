...
final class Event_Repository {
	 ...
	 *
	 * @param iterable $events Normalized events to consume.
	 */
	...
	public function clear_source( string $source ): int {
		$deleted = 0;
		$this->with_write_lock(
			function () use ( $source, &$deleted ): void {
				$this->begin();
				try {
						$result = $this->database->delete( $this->table, array( 'source' => $source ), array( '%s' ) );
					if ( false === $result ) {
						throw new RuntimeException( 'The selected request log could not be cleared.' );
					}
					$deleted = (int) $result;
					$this->commit();
				} catch ( \Throwable $error ) {
		...
		$this->with_write_lock(
			function (): void {
				...
				try {
					...
					if ( false === $this->database->query( $this->database->prepare( $sql, gmdate( 'Y-m-d H:i:s', time() - HOUR_IN_SECONDS * $hours ) ) ) ) {
						throw new RuntimeException( 'Expired request events could not be removed.' );
					}
					$this->enforce_cap();
					$this->commit();
				} catch ( \Throwable $error ) {
	 ...
	 *
	 * @param array $batch Bounded normalized provider batch.
	 * @throws RuntimeException When the operation cannot complete safely.
	 */
	private function insert_provider_batch( array $batch ): int {
		$inserted = 0;
		$this->with_write_lock(
			function () use ( $batch, &$inserted ): void {
				$this->begin();
				try {
	...
	/**
	 * Start a transaction for the current writer.
	 *
	 * @throws RuntimeException When the operation cannot complete safely.
	 */
	private function begin(): void {
		if ( false === $this->database->query( 'START TRANSACTION' ) ) {
			throw new RuntimeException( 'The request log transaction could not start.' );
		}
	}

	/**
	 * Commit the current writer transaction.
	 *
	 * @throws RuntimeException When the operation cannot complete safely.
	 */
	private function commit(): void {
		if ( false === $this->database->query( 'COMMIT' ) ) {
			throw new RuntimeException( 'The request log transaction could not commit.' );
		}
	}

	/**
	 * Roll back the current writer transaction.
	 *
	 * @throws RuntimeException When the operation cannot complete safely.
	 */
	private function rollback(): void {
		if ( false === $this->database->query( 'ROLLBACK' ) ) {
			throw new RuntimeException( 'The request log transaction could not roll back.' );
		}
	}

	/**
	 * Attempt rollback while preserving the original operation error.
	 *
	 * @param \Throwable $original Original operation failure.
	 * @throws \Throwable When the operation cannot complete safely.
	 */
	private function rollback_preserving( \Throwable $original ): void {
		try {
			$this->rollback();
		} catch ( \Throwable $rollback_error ) { // phpcs:ignore Generic.CodeAnalysis.EmptyStatement.DetectedCatch -- Preserve the original operation error if rollback also fails.
			/* Preserve the operation failure. */ }
			throw $original;
	}

	/**
	 * Serialize a writer and release its advisory lock exactly once.
	 *
	 * @param callable $operation Operation executed while holding the writer lock.
	 * @throws RuntimeException When the operation cannot complete safely.
	 */
	private function with_write_lock( callable $operation ): void {
		$database = $this->database->get_var( 'SELECT DATABASE()' );
		if ( ! is_string( $database ) || '' === $database ) {
			throw new RuntimeException( 'The request log database identity is unavailable.' );
		}
		...
		if ( '1' !== (string) $acquired ) {
			throw new RuntimeException( 'The request log write lock is unavailable.' );
		}
		$failure = null;
		try {
			$operation();
