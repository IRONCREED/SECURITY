...
final class Repository_Database extends wpdb {
	...
	public function get_var( $query ) { return str_contains( is_array( $query ) ? $query[0] : $query, 'GET_LOCK(' ) ? ( $this->lock_available ? 1 : 0 ) : ( str_contains( is_array( $query ) ? $query[0] : $query, 'RELEASE_LOCK(' ) ? 1 : null ); }
	...
	public function query( $query ) {
		if ( is_string( $query ) ) { $this->transactions[] = $query; if ( 'START TRANSACTION' === $query ) $this->snapshot = $this->rows; if ( 'ROLLBACK' === $query ) $this->rows = $this->snapshot; return true; }
