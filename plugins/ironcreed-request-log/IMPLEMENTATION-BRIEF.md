...
фиксированный `get_services` с `type=host`, получает доступные token хостинговые
сайты и локально сопоставляет введённый домен с полем `host`. Сначала применяется
точное совпадение; при его отсутствии допустим только единственный эквивалент с
одним ведущим `www.`. Поле `id` единственной подходящей услуги становится
positive `host_id`. Domain не передаётся отдельным lookup-параметром, а полученный
список discovery не сохраняется. `account_id`, `virtual_domain_id`, User ID и
WordPress blog ID не подставляются вместо `host_id`. Неоднозначное или отсутствующее
совпадение завершается безопасным отказом. Connection сохраняется только после
явного submit. Ручное сохранение не вызывает HTTP. Для Multisite предлагается домен
главного сайта; администратор сверяет virtual host.
