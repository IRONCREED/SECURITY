...
Получение remote log запускается вручную либо после отдельного явного согласия
на периодический импорт. Интервал, следующий запуск и результат видимы;
ошибки ограничивают частоту, disable/disconnect/deactivation снимают задачи.
Параметры даты и сайта
