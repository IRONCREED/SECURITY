...
final class RepositoryTest extends TestCase {
	protected function setUp(): void { $GLOBALS['test_options'] = array(); }
	...
	public function test_cleanup_failures_restore_rows_and_preserve_error(): void {
		$db = new Repository_Database(); $db->rows[] = $this->event( 'wordpress-runtime', '' ) + array( 'event_id' => 1 ); $db->fail_retention = true;
	...
	public function test_runtime_and_clear_failure_matrix_restores_rows(): void {
		$db = new Repository_Database(); $db->fail_insert = true;
		try { ( new Event_Repository( $db ) )->insert_runtime( $this->event( 'wordpress-runtime', '' ) ); self::fail( 'Expected Runtime failure.' ); } catch ( RuntimeException $error ) { self::assertSame( 'The request event could not be stored.', $error->getMessage() ); }
		self::assertSame( array(), $db->rows ); self::assertSame( array( 'START TRANSACTION', 'ROLLBACK' ), $db->transactions ); self::assertSame( 1, $db->release_attempts );

		foreach ( array( 'delete', 'COMMIT' ) as $failure ) {
			$db = new Repository_Database(); $db->rows[] = $this->event( 'hosting-ukraine-nginx', str_repeat( 'f', 64 ) ) + array( 'event_id' => 1 );
			if ( 'delete' === $failure ) $db->fail_delete_after_mutation = true; else $db->fail_transaction = 'COMMIT';
			try { ( new Event_Repository( $db ) )->clear_source( 'hosting-ukraine-nginx' ); self::fail( 'Expected clear failure.' ); } catch ( RuntimeException $error ) { self::assertNotSame( '', $error->getMessage() ); }
			self::assertCount( 1, $db->rows ); self::assertContains( 'ROLLBACK', $db->transactions ); self::assertSame( 1, $db->release_attempts );
		}
	}
	...
	public function test_cleanup_prune_and_commit_failures_restore_retention_mutation(): void {
		foreach ( array( 'prune', 'COMMIT' ) as $failure ) {
			$GLOBALS['test_options']['ironcreed_request_log_cap'] = 100;
			$db = new Repository_Database();
			for ( $i = 0; $i < 102; ++$i ) {
				$row = $this->event( 'wordpress-runtime', '' ) + array( 'event_id' => $i + 1 );
				$row['observed_at'] = 0 === $i ? '2000-01-01 00:00:00' : gmdate( 'Y-m-d H:i:s' ); $db->rows[] = $row;
			}
			$before = $db->rows;
			if ( 'prune' === $failure ) $db->fail_prune = true; else $db->fail_transaction = 'COMMIT';
			try { ( new Event_Repository( $db ) )->cleanup(); self::fail( 'Expected cleanup failure.' ); }
			catch ( RuntimeException $error ) { self::assertStringContainsString( 'prune' === $failure ? 'hard cap' : 'transaction', $error->getMessage() ); }
			self::assertSame( $before, $db->rows ); self::assertSame( 'START TRANSACTION', $db->transactions[0] ); self::assertSame( 'ROLLBACK', end( $db->transactions ) ); self::assertSame( 1, $db->release_attempts );
		}
	}
...
final class Repository_Database extends wpdb {
	...
	public bool $fail_delete_after_mutation = false;
	...
	public function insert( $table, $event, $formats ) { if ( $this->fail_insert ) return false; $event['event_id'] = $this->next_id++; $this->rows[] = $event; return 1; }
	public function delete( $table, $where, $formats ) { $this->rows = array_values( array_filter( $this->rows, static fn( array $row ): bool => $where['source'] !== $row['source'] ) ); return $this->fail_delete_after_mutation ? false : 1; }
	public function prepare( $sql, ...$args ) { if ( 1 === count( $args ) && is_array( $args[0] ) ) $args = $args[0]; return array( $sql, $args ); }
	...
	public function query( $query ) {
		...
		if ( str_starts_with( $query[0], 'DELETE FROM' ) ) {
			if ( str_contains( $query[0], 'LIMIT 5000' ) ) { $before = count( $this->rows ); $time = $query[1][0]; $this->rows = array_values( array_filter( $this->rows, static fn( $row ) => $row['observed_at'] >= $time ) ); if ( $this->fail_retention ) return false; return $before - count( $this->rows ); }
