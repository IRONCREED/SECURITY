...
final class MigrationTest extends TestCase {
	...
	public function test_myisam_conversion_and_safe_failures(): void {
		$this->prepare(); $db = new Migration_Database(); $db->engines = array( 'MyISAM', 'InnoDB' ); $GLOBALS['wpdb'] = $db; self::assertTrue( Lifecycle::maybe_upgrade() ); self::assertStringContainsString( 'ENGINE=InnoDB', implode( '\n', $db->queries ) );
	...
	private function prepare(): void { @mkdir( ABSPATH . 'wp-admin/includes', 0777, true ); @file_put_contents( ABSPATH . 'wp-admin/includes/upgrade.php', "<?php\n" ); }
}
