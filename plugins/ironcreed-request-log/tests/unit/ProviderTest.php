...
final class ProviderTest extends TestCase {
	...
	public function test_limit_policy_rejects_unknown_and_unsafe_values(): void {
		foreach ( array( array( 'unknown' => 1 ), array( 'line' => 0 ), array( 'records' => -1 ), array( 'compressed' => PHP_INT_MAX ), array( 'line' => '1' ) ) as $limits ) {
			try {
				new Hosting_Ukraine_Provider( new Synthetic_HTTP_Client( 200, gzencode( '' ) ), $limits );
				self::fail( 'Expected invalid policy failure.' );
			} catch ( RuntimeException $error ) {
				self::assertSame( 'The provider limit policy is invalid.', $error->getMessage() );
			}
		}
	}
